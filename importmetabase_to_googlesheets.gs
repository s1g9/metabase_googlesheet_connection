...
function refreshactivesheet() {
    ...
    Metabaseconnection.getQuestionAsCSV(questionNumber,sheetName);
}


function importQuestion(metabaseQuestionNum) {
  var metabaseQuestionNum = Browser.inputBox('Metabase question number (This will replace all data in the current tab with the result)','Enter x from (metabase/x)', Browser.Buttons.OK_CANCEL);
...
function importAllQuestions(automated) {
  ...
  function fillSheet(values, sheetName) {
    ...
    var clearrange= sheet.getRange(minCol+":"+maxCol);
